...
const RecruiterDetailView = ({ recruiter, onClose, onEdit, onDelete }) => {
  ...
  const fetchRecruiterDetails = async () => {
    try {
      ...
      const response = await axios.get(`https://test.airuter.com/api/admin/recruiters/${recruiter._id}`, {
  ...
  const handleStatusUpdate = async () => {
    try {
      ...
      const response = await axios.put(
        `https://test.airuter.com/api/admin/recruiters/${recruiter._id}/status`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
