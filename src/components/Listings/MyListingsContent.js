...
const JobCard = ({ job, onView, onEdit, onDelete, onViewApplications, onPreview, onToggleVisibility }) => {
  ...
  // Get applicant count from the correct property
  const applicantCount = job.applicationStats?.total || 0

  const getStatusColor = (status) => {
    switch (status) {
      case "active":
        return isDark
          ? "from-emerald-950 to-emerald-900/50 shadow-emerald-900/20"
          : "from-emerald-50 to-white shadow-emerald-200/30"
      case "hidden":
        return isDark
          ? "from-amber-950 to-amber-900/50 shadow-amber-900/20"
          : "from-amber-50 to-white shadow-amber-200/30"
      case "closed":
        return isDark ? "from-rose-950 to-rose-900/50 shadow-rose-900/20" : "from-rose-50 to-white shadow-rose-200/30"
      case "expired":
        return isDark
          ? "from-orange-950 to-orange-900/50 shadow-orange-900/20"
          : "from-orange-50 to-white shadow-orange-200/30"
      case "draft":
      default:
        return isDark ? "from-slate-900 to-slate-800 shadow-slate-900/20" : "from-slate-50 to-white shadow-slate-200/30"
    }
  }
  ...
  const getBorderColor = (status) => {
    switch (status) {
      case "active":
        return isDark ? "border-emerald-800" : "border-emerald-200"
      case "hidden":
        return isDark ? "border-amber-800" : "border-amber-200"
      case "closed":
        return isDark ? "border-rose-800" : "border-rose-200"
      case "expired":
  ...
  return (
    <div
      className={`relative rounded-xl overflow-hidden bg-gradient-to-br ${getStatusColor(job.status)} border ${getBorderColor(job.status)} shadow-lg hover:shadow-xl transition-all duration-300 mb-6`}
    >
      <div className="p-6">
        <div className="flex justify-between items-start">
          <div className="flex-1">
            ...
            <div className="space-y-2 mb-4">
              ...
              {/* Fixed applicant count display */}
              <div className={`flex items-center ${colors.textMuted}`}>
                <Briefcase className="h-4 w-4 mr-2" />
                <span className="text-sm">{applicantCount} applicants</span>
              </div>

              {/* Add more detailed application stats if available */}
              {job.applicationStats && job.applicationStats.total > 0 && (
                <div className={`flex items-center gap-4 text-xs ${colors.textMuted} mt-2`}>
                  <span className="flex items-center">
                    <div className="w-2 h-2 bg-yellow-500 rounded-full mr-1"></div>
                    {job.applicationStats.pending || 0} pending
                  </span>
                  <span className="flex items-center">
                    <div className="w-2 h-2 bg-blue-500 rounded-full mr-1"></div>
                    {job.applicationStats.reviewed || 0} reviewed
                  </span>
                  <span className="flex items-center">
                    <div className="w-2 h-2 bg-green-500 rounded-full mr-1"></div>
                    {job.applicationStats.shortlisted || 0} shortlisted
                  </span>
                </div>
              )}
            </div>
      ...
      <div className={`absolute top-4 right-4 flex gap-2 z-10`}>
        ...
        <button
          ...
          className={`p-2 rounded-full ${
            isDark ? "bg-rose-900/30 text-rose-300 hover:bg-rose-800/50" : "bg-rose-50 text-rose-500 hover:bg-rose-100"
          } backdrop-blur-sm transition-colors`}
        >
...
const EmptyState = ({ onPostJob, isDark, colors }) => (
  ...
  >
    ...
    <p className={`${colors.textMuted} mb-6 max-w-md mx-auto`}>
      Start attracting top talent by creating your first job listing. You can post jobs, review applications, and manage
  ...
  return (
    <div className={`rounded-xl ${scheme.bg} p-4 border ${scheme.border} relative overflow-hidden`}>
      <div className="flex justify-between">
        ...
        <div className={`${scheme.iconBg} p-3 rounded-xl ${scheme.iconColor}`}>{icon}</div>
      </div>
...
const MyListingsContent = () => {
  ...
  const handleToggleVisibility = async (jobId, newStatus) => {
    try {
      ...
      setListings(listings.map((job) => (job._id === jobId ? { ...job, status: updatedJob.status } : job)))
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96">
        <div
          className={`w-16 h-16 rounded-full border-4 border-t-transparent ${isDark ? "border-purple-600" : "border-purple-500"} animate-spin mb-4`}
        ></div>
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6">
      ...
      >
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div className="flex items-center">
            ...
            <div>
              <h1 className={`text-2xl font-bold ${isDark ? "text-white" : "text-gray-800"}`}>My Job Listings</h1>
              <p className={colors.textMuted}>Manage your job postings and view applications</p>
            </div>
          </div>

          <div className="flex items-center gap-4">
            <div
              className={`flex p-1 rounded-lg ${isDark ? "bg-gray-800 border border-gray-700" : "bg-gray-100 border border-gray-200"}`}
            >
              <button
        ...
        {listings.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-xl border shadow-sm mb-6 overflow-x-auto">
            <table
              className={`min-w-full divide-y ${isDark ? "divide-gray-700 border-gray-700" : "divide-gray-200 border-gray-200"}`}
            >
              <thead className={isDark ? "bg-gray-800" : "bg-gray-50"}>
                <tr>
                  <th
                    className={`px-6 py-3.5 text-left text-xs font-semibold ${isDark ? "text-gray-300" : "text-gray-500"} uppercase tracking-wider`}
                  >
                    Job Title
                  </th>
                  <th
              ...
              <tbody
                className={`${isDark ? "bg-gray-900" : "bg-white"} divide-y ${isDark ? "divide-gray-700" : "divide-gray-200"}`}
              >
                {sortedListings.map((job) => {
                  const getBorderClass = (status) => {
                    switch (status) {
                      case "active":
                        return isDark ? "border-l-4 border-emerald-600" : "border-l-4 border-emerald-500"
                      case "hidden":
                        return isDark ? "border-l-4 border-amber-600" : "border-l-4 border-amber-500"
                      case "closed":
                        return isDark ? "border-l-4 border-rose-600" : "border-l-4 border-rose-500"
                      case "expired":
                        return isDark ? "border-l-4 border-orange-600" : "border-l-4 border-orange-500"
                      case "draft":
                        return isDark ? "border-l-4 border-slate-600" : "border-l-4 border-slate-400"
                      default:
                        return ""
                    }
                  }
                  ...
                  return (
                    <tr
                      key={job._id}
                      className={`${getBorderClass(job.status)} ${isDark ? "hover:bg-gray-800" : "hover:bg-gray-50"} transition-colors`}
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          ...
                          />
                          <span className={`font-medium ${isDark ? "text-white" : "text-gray-900"}`}>{job.title}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <StatusBadge status={job.status} />
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="flex items-center space-x-2 ml-2">
                            <button
                              onClick={() =>
                                handleToggleVisibility(job._id, job.status === "hidden" ? "active" : "hidden")
                              }
                              className={`p-2 rounded-full ${
      ...
      <AlertDialog open={!!deleteJobId} onOpenChange={() => setDeleteJobId(null)}>
        <AlertDialogContent
          className={`${isDark ? "bg-gray-900 border-gray-700" : "bg-white border-gray-200"} rounded-xl border shadow-xl`}
        >
          <AlertDialogHeader>
            <AlertDialogTitle className={isDark ? "text-white" : "text-gray-900"}>Confirm Deletion</AlertDialogTitle>
            <AlertDialogDescription className={isDark ? "text-gray-300" : "text-gray-600"}>
              This will permanently delete the job listing and all associated applications. This action cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
              className={`${isDark ? "bg-gray-800 hover:bg-gray-700 text-gray-200 border-gray-700" : "bg-white hover:bg-gray-50 text-gray-700 border-gray-200"} rounded-lg border`}
            >
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction onClick={handleDelete} className="bg-rose-600 hover:bg-rose-700 text-white rounded-lg">
              Delete Job
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
